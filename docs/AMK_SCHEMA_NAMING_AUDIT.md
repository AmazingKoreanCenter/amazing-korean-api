...
| # | 발견 | 위치 | 영향 | 조치 |
|---|---|---|---|---|
| ~~AUD-1~~ ✅ | ~~🐛 rekey 도구 PK 표기 오류~~ **해결 (2026-05-19)** | `src/bin/rekey_encryption.rs:80` `pk:"oauth_id"` → `pk:"user_oauth_id"` | ~~users_oauth 키 로테이션 실행 시 실패~~ | 1줄 정정. 4 RekeyTarget pk 전수 실측(users/users_oauth/login/login_log 전부 OK=형제 버그 0) + 정정 쿼리 live DB 유효 + `cargo check --locked` 0 |
...
## 11. 그룹 ④ explanation→study_contents — 현황 분석 + R1/R3 (2026-05-19, R1 미결정·다음 세션)

> AUD-1·KKRYOUN 충돌 해소 후 그룹 ④ 착수 전 정밀 현황 분석. **R1 결정은 다음 세션(사용자: "내일")으로 보류.**

### 11.1 현황 (전 클라이언트 리포 실측)

이중모델 여전히 미통합:
- 레거시 `study_explain` = **0행**(local·prod). 서빙 `GET /studies/tasks/{id}/explain`(`study/repo.rs:533 FROM study_explain`) + admin CRUD 5. 소비 = api프론트 `study_api.ts:57` + **desktop** `study_api.ts:48`/ExplainCard, **mobile 0**. → 학습자 호출 시 빈 응답 = **해설 0건 노출**.
- 신 `explanation_unit`(568)/`explanation_block`(1317) + `content_translations` **4362행**(content_type IN explanation_unit/block). 서빙 `GET /explanations`(explanation 모듈). **소비처 전무** — api프론트·mobile·desktop **0**. prod 적재(#298)됐으나 아무도 안 봄.
- enum 5종 연계: `content_type_enum`(값 explanation_unit/explanation_block) + `explanation_unit_kind_enum`·`explanation_source_enum`·`explanation_block_type_enum`.

### 11.2 R3 — `/explanations` 라우트 개명 → **확정: R3-a (저비용)**

`/explanations` 호출 클라이언트 **전무**(api프론트·mobile·desktop 0, 실측). 개명해도 계약 파손 0 → `/explanations`→`/study-contents` 개명 채택(테이블·모듈·enum 일관). 비용 ≈ 0.

### 11.3 R1 — study_explain 처분 (핵심 스코프, **미결정·다음 세션**)

| 옵션 | 내용 | 비용 | 학습자 가치 |
|---|---|---|---|
| **A2** 순수 리네임만 (이전 전체 스코프 'A' 정합) | explanation_*→study_contents 리네임(INC-004)만, study_explain·배선 불변, 통합 후속 별도 | 🟡 저 (~0.5~1일, ①②③+enum값 패턴) | 0 (해설 0건 잔존·이중모델 churn) |
| **A1** 리네임 + study_explain 폐기 | +study_explain DROP+학습/admin/프론트·desktop 배선 제거 | 🟠 중 (계약파손 web·desktop, admin 해설관리 소멸) | 0 (제거만) |
| **A3** 통합 후 리네임 (트랙 본질) | 학습/admin/프론트·desktop를 study_contents 조회 재배선 + study_explain 흡수 + 리네임 | 🔴 최대 (기능+INC-004+2클라리포, 멀티데이) | 높음 (학습자 실해설 최초 노출) |

평가: A2=안전·저비용·가치0·churn위험 / A3=고비용·본질·유일 가치전달(메모리 "통합 후 리네임=본질, 리네임 선행 시 churn"과 정합) / A1=어중간(가치0+계약파손) 비권장.

### 11.4 INC-004 리네임 기계 (R1 무관 공통)

`ALTER TABLE explanation_unit/block RENAME`(메타데이터, ①②③ 동일+**존재 가드** 기본) + enum 타입 3 `ALTER TYPE RENAME` + `content_type_enum` **값** 2개 `ALTER TYPE ... RENAME VALUE`(4362행 라벨 자동, 코드 문자열 lockstep: types.rs serde/repo/seed) + `.sqlx` 재생성 + fresh-DB CI 동일 psql 재현 검증. 신규 forward 마이그만(INC-004 차단).

**다음 세션 진입 = R1(A2/A3) 결정 → 그룹 ④ 실행.** R3-a 확정.
