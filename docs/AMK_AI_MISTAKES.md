...
| 카테고리 | 사고 ID | 공통 행위 |
|----------|---------|----------|
| 사전 상태 미확인 | M-001, M-003, M-004, **M-008**, **M-010**, **M-012** | 작업 시작 전/검증 시 현재 상태 (origin / git status / baseline / CI step / 외부 API / **실제 적용 경로·전역 범위**) 확인 누락. M-012 = 검증을 진짜 경로(`sqlx migrate run`) 대신 대용물(`psql`)로 + 문서 sweep 범위를 판단으로 한정 |
