...
CREATE TYPE user_language_enum AS ENUM ('ko', 'en', 'ja', 'zh_cn', 'zh_tw', 'vi', 'th', 'id', 'my', 'mn', 'ru', 'es', 'pt', 'fr', 'de', 'hi', 'ne', 'si', 'km', 'uz', 'kk', 'tg');  -- 22개 언어 확장 (기존: 'ko', 'en')
...
CREATE TYPE user_set_language_enum AS ENUM ('ko', 'en', 'ja', 'zh_cn', 'zh_tw', 'vi', 'th', 'id', 'my', 'mn', 'ru', 'es', 'pt', 'fr', 'de', 'hi', 'ne', 'si', 'km', 'uz', 'kk', 'tg');  -- 22개 언어 확장 (기존: 'ko', 'en')
...
CREATE TYPE content_type_enum AS ENUM ('course', 'lesson', 'video', 'video_tag', 'study', 'study_task_choice', 'study_task_typing', 'study_task_voice', 'study_task_writing');
...
CREATE TABLE admin_action_log (
  ...
  ip_address TEXT,                                              -- 접속 IP : 관리자 접속 IP -- encrypted (AES-256-GCM)
...
CREATE TABLE users (
  ...
  user_email TEXT NOT NULL,                                      -- 사용자 이메일 : 사용자 로그인 시 사용하는 아이디(변경불가) -- encrypted (AES-256-GCM)
  user_email_idx TEXT NOT NULL,                                  -- 사용자 이메일 블라인드 인덱스 : HMAC-SHA256 검색용
  user_password text,                                            -- 사용자 비빌번호 : 암호화 후 DB 저장, 복호화를 통해 사용 (OAuth 사용자는 NULL)
  user_name TEXT NOT NULL,                                       -- 사용자 이름 : 모든 언어로 저장 가능 -- encrypted (AES-256-GCM)
  user_name_idx TEXT NOT NULL,                                   -- 사용자 이름 블라인드 인덱스 : HMAC-SHA256 검색용
  ...
  user_birthday TEXT NOT NULL,                                   -- 사용자 생년월일 : 사용자 기본 정보 -- encrypted (AES-256-GCM)
...
CREATE TABLE users_log (
  ...
  user_birthday_log TEXT,                                            -- 사용자 생년월일 변경 기록 : 생년월일 변경 기록 -- encrypted (AES-256-GCM)
...
-- 2.1.3 users_oauth : OAuth 연동 정보
CREATE TABLE users_oauth (
  user_oauth_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  user_id BIGINT NOT NULL,
  oauth_provider login_method_enum NOT NULL,
  oauth_subject TEXT NOT NULL,
  oauth_subject_idx TEXT NOT NULL,
  oauth_email TEXT,
  oauth_name VARCHAR(255),
  oauth_picture_url TEXT,
  oauth_linked_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  oauth_last_login_at TIMESTAMPTZ
);

-- 2.1.4 users_setting : 사용자 설정
...
-- 2.1.4 users_export_data : 사용자 요청 데이터(학습 이력, 수료증 등)
CREATE TABLE users_export_data (
...
CREATE TABLE login (
  ...
  login_ip TEXT NOT NULL,                                        -- 로그인 IP : 로그인 Internet Protocol 정보 -- encrypted (AES-256-GCM)
...
--login_id fk 추가?  
  ...
  login_ip_log TEXT,                                                 -- 로그인 ip 기록 : 로그인 ip 기록 -- encrypted (AES-256-GCM)
...
-- 2.4.5-1 study_task_writing : 학습 문제 [한글 자판 연습/쓰기] (2026-04-12 추가)
-- 초급(자모/음절/단어) → 중급(단어/문장) → 고급(문장/문단)
CREATE TABLE study_task_writing (
  study_task_id int PRIMARY KEY,                                              -- 학습 문제 id : 학습 문제 DB 고유 번호(PK, FK)
  study_task_writing_level writing_level_enum NOT NULL DEFAULT 'beginner',    -- 레벨 : beginner(초급), intermediate(중급), advanced(고급)
  study_task_writing_practice_type writing_practice_type_enum NOT NULL DEFAULT 'word',  -- 연습 유형 : jamo(자모), syllable(음절), word(단어), sentence(문장), paragraph(문단)
  study_task_writing_prompt text NOT NULL,                                    -- 표시 텍스트 : 학습자에게 보여줄 타이핑 대상 텍스트
  study_task_writing_answer text NOT NULL,                                    -- 정답 : 서버 채점용 정답 텍스트
  study_task_writing_hint text,                                               -- 힌트 : 로마자/설명 등 보조 힌트 (선택)
  study_task_writing_keyboard_visible boolean NOT NULL DEFAULT true,          -- 키보드 기본 표시 : 가상 키보드 초기 표시 여부
  study_task_writing_image_url text,                                          -- 이미지 URL : 문제용 이미지 (선택)
  study_task_writing_audio_url text                                           -- 오디오 URL : 발음 가이드 오디오 (선택)
);

-- 2.4.5-2 study_writing_practice_session : 한글 자판 연습 세션 통계 (2026-04-12 추가, 20260520 리네임)
-- 사용자의 실제 연습 세션 단위로 통계 저장 (정확도/CPM/취약 글자 분석)
CREATE TABLE study_writing_practice_session (
  session_id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,    -- 세션 id : 연습 세션 DB 고유 번호(PK)
  user_id bigint NOT NULL,                                           -- 사용자 id : 사용자 DB 고유 번호(FK)
  study_task_id int,                                                 -- 학습 문제 id : 연결된 문제(FK, 자유 연습 시 NULL)
  writing_level writing_level_enum NOT NULL,                         -- 레벨 : 세션 수행 시점의 레벨
  writing_practice_type writing_practice_type_enum NOT NULL,         -- 연습 유형 : 세션 수행 시점의 유형
  started_at timestamptz NOT NULL DEFAULT (now()),                   -- 시작 시간 : 세션 시작 시각
  finished_at timestamptz,                                           -- 종료 시간 : 세션 완료 시각 (진행 중이면 NULL)
  total_chars int NOT NULL DEFAULT 0,                                -- 총 글자 수 : 입력한 전체 글자 수
  correct_chars int NOT NULL DEFAULT 0,                              -- 정답 글자 수 : 정확히 입력한 글자 수
  accuracy_rate numeric(5,2) NOT NULL DEFAULT 0.00,                  -- 정확도(%) : 0.00 ~ 100.00
  chars_per_minute numeric(7,2) NOT NULL DEFAULT 0.00,               -- 분당 타수(CPM) : 분당 글자 수
  mistakes jsonb NOT NULL DEFAULT '[]'::jsonb                        -- 오타 목록 : [{position, expected, actual}] 취약 글자 분석용
);

-- 2.4.5-3 study_writing_practice_seed : 자유 연습 드릴 컨텐츠 (2026-04-13 추가, 20260520 리네임)
-- study_task 수강 흐름과 독립된 시드 컨텐츠. 레벨+유형별 seq 순서대로 제공.
-- 초급(jamo 40 + syllable 30 + word 30), 중급(word 30 + sentence 30), 고급(sentence 20 + paragraph 10)
CREATE TABLE study_writing_practice_seed (
  writing_practice_seed_id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- 시드 id : 시드 컨텐츠 DB 고유 번호(PK)
  writing_level writing_level_enum NOT NULL,                                     -- 레벨 : beginner/intermediate/advanced
  writing_practice_type writing_practice_type_enum NOT NULL,                     -- 연습 유형 : jamo/syllable/word/sentence/paragraph
  seq int NOT NULL,                                                              -- 순번 : (level, practice_type) 내 표시 순서
  prompt text NOT NULL,                                                          -- 표시 텍스트 : 학습자에게 보여줄 타이핑 대상
  answer text NOT NULL,                                                          -- 정답 : 입력해야 할 정답 (대부분 prompt와 동일)
  hint text,                                                                     -- 힌트 : 로마자/뜻 등 (선택)
...
-- 2.6.3 users_course : 사용자 수강권 (구매/부여된 코스)
...
-- 2.7 i18n table : 다국어 콘텐츠 번역 테이블

-- 2.7.1 content_translations : 콘텐츠 번역 (정규화된 다국어 번역 저장)
CREATE TABLE content_translations (
  translation_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,       -- 번역 id : 번역 DB 고유 번호(PK)
  ...
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),                            -- 번역 수정 시간 : 번역 정보 수정 시간
  UNIQUE (content_type, content_id, field_name, lang)                       -- 유니크 제약 : 동일 콘텐츠의 동일 필드에 대해 언어별 1개 번역만 허용
);
-- UPSERT 동작: ON CONFLICT 시 translated_text가 실제로 변경된 경우에만 status='draft'로 리셋.
--              동일 텍스트 재제출 시 기존 status 유지 (불필요한 상태 리셋 방지).

-- 2.8 payment table : 결제 테이블 (Paddle Billing)

-- Payment ENUM Types (2026-02-15)
CREATE TYPE payment_provider_enum AS ENUM ('paddle', 'apple', 'google', 'revenuecat');
CREATE TYPE subscription_status_enum AS ENUM ('trialing', 'active', 'past_due', 'paused', 'canceled');
CREATE TYPE transaction_status_enum AS ENUM ('completed', 'refunded', 'partially_refunded');
CREATE TYPE billing_interval_enum AS ENUM ('month_1', 'month_3', 'month_6', 'month_12');

-- 2.8.1 payment_subscription : 구독 정보 (Source of Truth) (20260521 리네임)
CREATE TABLE payment_subscription (
  subscription_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- 구독 id : 구독 DB 고유 번호(PK)
  user_id BIGINT NOT NULL,                                              -- 사용자 id : 구독 소유 사용자(FK)
  payment_provider payment_provider_enum NOT NULL DEFAULT 'paddle',     -- 결제 제공자 : paddle
  provider_subscription_id VARCHAR(255) NOT NULL,                       -- Provider 구독 ID : Paddle sub_xxx
  provider_customer_id VARCHAR(255),                                    -- Provider 고객 ID : Paddle ctm_xxx
  status subscription_status_enum NOT NULL DEFAULT 'trialing',          -- 구독 상태 : trialing/active/past_due/paused/canceled
  billing_interval billing_interval_enum NOT NULL,                      -- 결제 주기 : month_1/3/6/12
  current_price_cents INT NOT NULL,                                     -- 현재 가격 (센트) : 예 1000 = $10.00
  trial_started_at TIMESTAMPTZ,                                         -- 체험 시작일
  trial_ends_at TIMESTAMPTZ,                                            -- 체험 종료일
  current_period_start TIMESTAMPTZ,                                     -- 현재 기간 시작
  current_period_end TIMESTAMPTZ,                                       -- 현재 기간 종료
  canceled_at TIMESTAMPTZ,                                              -- 취소 시각
  paused_at TIMESTAMPTZ,                                                -- 일시정지 시각
  provider_data JSONB,                                                  -- Provider 원본 데이터
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),                        -- 구독 생성 시간
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()                         -- 구독 수정 시간
);

-- 2.8.2 payment_transaction : 결제 트랜잭션 기록 (20260521 리네임)
CREATE TABLE payment_transaction (
  transaction_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,   -- 트랜잭션 id : 트랜잭션 DB 고유 번호(PK)
  subscription_id BIGINT,                                               -- 구독 id : 관련 구독(FK)
  user_id BIGINT NOT NULL,                                              -- 사용자 id : 결제 사용자(FK)
  payment_provider payment_provider_enum NOT NULL DEFAULT 'paddle',     -- 결제 제공자
  provider_transaction_id VARCHAR(255) NOT NULL,                        -- Provider 트랜잭션 ID : Paddle txn_xxx
  status transaction_status_enum NOT NULL,                              -- 트랜잭션 상태 : completed/refunded/partially_refunded
  ...
  provider_data JSONB,                                                  -- Provider 원본 데이터
  occurred_at TIMESTAMPTZ NOT NULL,                                     -- 결제 발생 시간
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()                         -- 레코드 생성 시간
);

-- 2.8.3 payment_webhook_event : 웹훅 이벤트 추적 (멱등성 보장) (20260521 리네임)
CREATE TABLE payment_webhook_event (
  event_id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,         -- 이벤트 id : 웹훅 이벤트 DB 고유 번호(PK)
  payment_provider payment_provider_enum NOT NULL DEFAULT 'paddle',     -- 결제 제공자
  provider_event_id VARCHAR(255) NOT NULL,                              -- Provider 이벤트 ID : 중복 방지용
  event_type VARCHAR(100) NOT NULL,                                     -- 이벤트 타입 : subscription.activated 등
...
-- 2.9 textbook : 교재 주문 시스템 (2026-02-26)
...
CREATE TYPE textbook_payment_method_enum AS ENUM ('bank_transfer');

-- 2.9.1 textbook : 교재 주문 기본 정보
CREATE TABLE textbook (
  order_id         BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- 주문 id : PK
  order_code       VARCHAR(20) UNIQUE NOT NULL,                          -- 주문번호 : TB-YYMMDD-NNNN
  user_id          BIGINT REFERENCES users(user_id),                     -- 주문자 id : FK (NULLABLE — 기존 비회원 주문 호환) — 20260324 추가
  status           textbook_order_status_enum NOT NULL DEFAULT 'pending',-- 주문 상태
  ...
  org_type         VARCHAR(100),                                         -- 기관 유형
  delivery_postal_code VARCHAR(20),                                      -- 우편번호
  delivery_address TEXT NOT NULL,                                        -- 배송 주소
  delivery_detail  VARCHAR(200),                                         -- 상세 주소
  payment_method   textbook_payment_method_enum NOT NULL DEFAULT 'bank_transfer', -- 결제 방법
  ...
  notes            TEXT,                                                 -- 비고
  tracking_number  VARCHAR(100),                                         -- 배송 추적번호
  tracking_provider VARCHAR(100),                                        -- 택배사
...
CREATE TABLE textbook_item (
  item_id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- 항목 id : PK
  order_id         BIGINT NOT NULL REFERENCES textbook(order_id) ON DELETE RESTRICT, -- 주문 id : FK (RESTRICT — 항목 보존)
  textbook_language textbook_language_enum NOT NULL,                     -- 교재 언어
  textbook_type    textbook_type_enum NOT NULL,                          -- 교재 유형 : student/teacher
...
-- 2.9.3 admin_textbook_log : 관리자 교재 주문 작업 기록
CREATE TABLE admin_textbook_log (
  log_id           BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- 로그 id : PK
  admin_user_id    BIGINT NOT NULL REFERENCES users(user_id),            -- 관리자 id : FK
  order_id         BIGINT NOT NULL REFERENCES textbook(order_id) ON DELETE RESTRICT, -- 주문 id : FK (RESTRICT — 감사 로그 보존)
  action           admin_action_enum NOT NULL,                           -- 작업 유형
...
CREATE INDEX idx_textbook_email ON textbook(orderer_email);
CREATE INDEX idx_textbook_deleted ON textbook(is_deleted) WHERE is_deleted = true;
CREATE INDEX idx_textbook_user_id ON textbook(user_id);
CREATE INDEX idx_textbook_item_order ON textbook_item(order_id);

-- 2.10 E-book 웹 뷰어 도메인 (EBOOK) — 마이그레이션: 20260310_ebook.sql
...
CREATE TYPE ebook_payment_method_enum AS ENUM ('paddle', 'bank_transfer', 'apple_iap', 'google_iap');

-- 2.10.1 ebook_purchase : e-book 구매 기록 (회원 전용)
CREATE TABLE ebook_purchase (
  purchase_id       BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,  -- 구매 id : PK
  purchase_code     VARCHAR(30) UNIQUE NOT NULL,                          -- 구매 코드 : {LANG}-{ED}-{YYYYMMDD}-{PAY}-{NNNN}
  user_id           BIGINT NOT NULL REFERENCES users(user_id),            -- 사용자 id : FK (회원 전용)
  language          textbook_language_enum NOT NULL,                       -- 언어 : 기존 enum 재활용
  edition           ebook_edition_enum NOT NULL,                          -- 에디션 : student/teacher
  payment_method    ebook_payment_method_enum NOT NULL,                   -- 결제 방법
  status            ebook_purchase_status_enum NOT NULL DEFAULT 'pending', -- 구매 상태
  price             INT NOT NULL,                                         -- 가격 (KRW)
  currency          VARCHAR(3) NOT NULL DEFAULT 'KRW',                    -- 통화
  paddle_txn_id     VARCHAR(100),                                         -- Paddle transaction ID
  iap_platform      VARCHAR(10),                                          -- IAP 플랫폼 : 'apple' | 'google' | NULL
  iap_transaction_id VARCHAR(255),                                        -- IAP 스토어 transaction ID
  iap_product_id    VARCHAR(255),                                         -- IAP 스토어 product ID
  is_deleted        BOOLEAN NOT NULL DEFAULT false,                       -- Soft Delete 플래그
  deleted_at        TIMESTAMPTZ,                                          -- 삭제 시간
  completed_at      TIMESTAMPTZ,                                          -- 결제 완료 시간
  refunded_at       TIMESTAMPTZ,                                          -- 환불 시간
  created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),                   -- 생성 시간
  updated_at        TIMESTAMPTZ NOT NULL DEFAULT NOW()                    -- 수정 시간
);

-- 2.10.2 ebook_access_log : 페이지 조회 감사 로그
CREATE TABLE ebook_access_log (
  log_id           BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,   -- 로그 id : PK
  purchase_id      BIGINT NOT NULL REFERENCES ebook_purchase(purchase_id) ON DELETE RESTRICT, -- 구매 id : FK
  user_id          BIGINT NOT NULL REFERENCES users(user_id),             -- 사용자 id : FK
  page_number      INT NOT NULL,                                          -- 조회 페이지 번호
  watermark_id     VARCHAR(64),                                           -- 고유 워터마크 식별자
  ip_address       INET,                                                  -- 접속 IP
  user_agent       TEXT,                                                   -- User-Agent
  created_at       TIMESTAMPTZ NOT NULL DEFAULT NOW()                     -- 생성 시간
);

-- 2.10.3 admin_ebook_log : 관리자 e-book 작업 기록
...
ALTER TABLE users_export_data ADD FOREIGN KEY (user_id) REFERENCES users (user_id);
...
ALTER TABLE study_writing_practice_session ADD FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;
ALTER TABLE study_writing_practice_session ADD FOREIGN KEY (study_task_id) REFERENCES study_task (study_task_id) ON DELETE SET NULL;
...
ALTER TABLE users_course ADD FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;
ALTER TABLE users_course ADD FOREIGN KEY (course_id) REFERENCES course (course_id) ON DELETE CASCADE;
ALTER TABLE users_course ADD FOREIGN KEY (user_course_last_lesson_id) REFERENCES lesson (lesson_id);
ALTER TABLE users_course ADD FOREIGN KEY (user_course_granted_by_user_id) REFERENCES users (user_id);
...
ALTER TABLE admin_course_log ADD FOREIGN KEY (admin_pick_user_course_id) REFERENCES users_course (user_course_id);
ALTER TABLE payment_subscription ADD FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;
ALTER TABLE payment_transaction ADD FOREIGN KEY (subscription_id) REFERENCES payment_subscription (subscription_id);
ALTER TABLE payment_transaction ADD FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE;
...
CREATE INDEX index_export_data_user_id ON users_export_data (user_id);
...
CREATE INDEX idx_users_course_user_id ON users_course (user_id);
CREATE INDEX idx_users_course_course_id ON users_course (course_id);
CREATE INDEX idx_users_course_active ON users_course (user_course_active);
CREATE INDEX idx_users_course_expire ON users_course (user_course_expire_at);
CREATE UNIQUE INDEX unique_users_course ON users_course (user_id, course_id);
...
CREATE INDEX idx_admin_course_log_users_course ON admin_course_log (admin_pick_user_course_id);
...
CREATE INDEX idx_payment_subscription_user_id ON payment_subscription (user_id);
CREATE INDEX idx_payment_subscription_status ON payment_subscription (status);
CREATE UNIQUE INDEX unique_provider_subscription ON payment_subscription (payment_provider, provider_subscription_id);
...
CREATE UNIQUE INDEX idx_payment_transaction_provider_txn_id ON payment_transaction (provider_transaction_id);
CREATE INDEX idx_payment_webhook_event_type ON payment_webhook_event (event_type);
```