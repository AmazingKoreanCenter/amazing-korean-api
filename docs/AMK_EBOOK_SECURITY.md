...
| 7 | 행동 기반 봇/스크래퍼 탐지 | TOONRADER 2025~2026 고도화 (§2.5). 현재 Rate Limit + `X-Ebook-Viewer` 만 — 헤드리스/이상 행동 패턴 식별 부재 | Phase 1 옵션 A |
| 8 | 도용/공유 계정 다중 IP 감지 | TOONRADER 2025~2026 고도화 (§2.5). 단일 세션 강제는 ✅, 이상 IP 변동 로그·알림 부재 | Phase 1 옵션 A |
...
### 2.5 글로벌 벤치마크: 네이버웹툰 TOONRADER (2017~2026)

네이버웹툰 자체 개발 안티-파이러시 시스템. **비가시적 워터마크 + AI 이미지 매칭 + 행동/계정 탐지 + 사후 워크플로**의 4축 구조. 우리 e-book 보안 갭 분석 벤치마크.

#### 2.5.1 기술 구성

| 구성 요소 | 내용 | 도입 시기 |
|-----------|------|-----------|
| 비가시적 워터마크 | 사용자 식별 정보를 이미지에 삽입 → 최초 유출자 특정 | 2017.07 |
| TOONRADAR AI | 이미지 매칭 자동 추적. 1화 검사 30분 → 10분 단축 | 2018.06 |
| 도용 계정 단속 | 탈취/공유 계정 자동 식별 | 2025~2026 고도화 |
| 자동화 접근 탐지 | 헤드리스/봇/스크래퍼 차단 | 2025~2026 고도화 |
| 비정상 행동 패턴 감지 | 페이지 진행 속도·체류 시간 통계 분석 | 2025~2026 고도화 |
| 사후 워크플로 | DMCA takedown + CDN/호스팅 소환장 + 안티-파이러시 전담팀 | 운영 |

#### 2.5.2 검증된 성과 (2026-04-22 발표)

| 지표 | 효과 |
|------|------|
| 24h 내 유출 작품 수 | **약 90% 감소** |
| 유료 결제액 | 평균 **23% 증가** (개별 작품 최대 60%) |
| 유출 시도 계정당 평균 회차 | 1/10 → 운영자 비용 10배 |

#### 2.5.3 우리 시스템 갭

| 항목 | TOONRADER | AMK 현재 | 갭 |
|------|-----------|----------|-----|
| 비가시적 워터마크 | AI 매칭 | LSB + 마이크로도트 (✅) | 자동 검색·매칭 시스템 부재 |
| 봇/자동화 탐지 | JS 챌린지 + 행동 패턴 | `X-Ebook-Viewer` 헤더 + Rate Limit | **헤드리스/스크래퍼 식별 부재** |
| 비정상 행동 패턴 | 시계열 분석 | 세션 heartbeat 30s | **행동 시계열 분석 부재** |
| 도용/공유 계정 | 다중 IP·디바이스 동시 탐지 | 세션 1개 강제 (✅) | 이상 IP 변동 로그·알림 부재 |
| 사후 추적 | 워터마크 → 유출자 → DMCA | `accesslog` 적재 (✅) | 외부 유출본 매칭 워크플로 부재 |
| 콘텐츠 모니터링 | 안티-파이러시 전담팀 | — | 1인 운영 한계 |

#### 2.5.4 출처

- 서울경제 (2025-12): https://www.sedaily.com/article/20035523
- 아시아경제 (2025-12, 80% 수치): https://www.asiae.co.kr/article/2025121810315627622
- Seoul Economic Daily EN (2026-04): https://en.sedaily.com/culture/2026/04/22/naver-webtoons-toon-radar-curbs-piracy-lifts-paid-revenue
- Korea Herald: https://www.koreaherald.com/article/10728309
- Anime News Network (2026-04, 23% 매출): https://www.animenewsnetwork.com/news/2026-04-26/naver-webtoon-reports-23-percent-revenue-boost-as-anti-piracy-system-delays-illegal-distribution/.236660
- 머니투데이 (2026-04, 90% 수치): https://www.mt.co.kr/tech/2026/04/22/2026042209062416491
...
| 1-6 | 행동 기반 봇 탐지 (옵션 A) — 헤드리스 시그널 + 페이지 요청 간격 통계 + 다중 IP/UA 동시 접속 알림 | TOONRADER 2025~2026 고도화 (§2.5) | 높음 (권장) |
...
- **TOONRADER 벤치마크 대비 격차** (2026-05-03 조사): 워터마크 ✅ / 단일 세션 ✅ / accesslog 적재 ✅ 갖췄으나 **자동화 봇 탐지 + 행동 패턴 시계열 분석 + 외부 매칭 워크플로 부재** (§2.5.3). Phase 1 옵션 A 부터 단계적 채움. 옵션 B (외부 모니터링) 는 1인 운영 한계로 보류, 옵션 C (AI 매칭) 는 콘텐츠 규모 대비 과투자로 비권장
