...
| 전환 | 조건 | 결정권자 |
|------|------|----------|
| 기획 → 검증 | 기획 문서 작성 완료 | 기획 AI |
| 검증 → 지시 | 검증 통과 + 사용자 승인 | **사용자** |
| 검증 → 기획 (반복) | 검증에서 문제 발견 | 기획 AI / 사용자 |
| 지시 → 실행 | 사용자가 작업 지시 | **사용자** |
| 실행 → 리뷰 | 구현 완료 + 빌드/테스트 통과 | 실행 AI |
| 리뷰 → QA | 코드 리뷰 승인 | 기획 AI / 사용자 |
| 리뷰 → 실행 (반복) | 리뷰에서 수정 요청 | 기획 AI / 사용자 |
| QA → 배포 | QA 통과 + 사용자 승인 | **사용자** |
| QA → 실행 (반복) | QA에서 버그 발견 | 실행 AI / 사용자 |
