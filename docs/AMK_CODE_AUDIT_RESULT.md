...
#### 카테고리 A: 함수 인자 과다 (too many arguments) — 16건
...
| 16 | `src/api/payment/repo.rs` | 146, 229 | 13/7, 12/7 | |
| 17 | `src/api/textbook/repo.rs` | 113 | **24/7** | 최다 인자 |
...
| 카테고리 | 건수 | 자동 수정 | 우선순위 |
|----------|------|-----------|----------|
| A. 함수 인자 과다 | 17 | 불가 | 리팩토링 |
| B. 불필요한 변환/복사 | 11 | **가능** | 즉시 |
| C. 코드 개선 제안 | 10 | **가능** | 즉시 |
| D. 구조적 경고 | 2 | 수동 | 권장 |
| **합계** | **42** (중복 제거) | **20건 자동** | — |
...
| # | 크레이트 | 위치 | 비고 |
|---|----------|------|------|
| 1 | `aes-gcm` | 루트 `Cargo.toml` | crypto 워크스페이스 멤버에서만 사용, 루트에서 직접 사용 없음 |
| 2 | `hmac` | 루트 `Cargo.toml` | crypto 워크스페이스 멤버에서만 사용, 루트에서 직접 사용 없음 |

**판정**: 워크스페이스 추출 시 루트에 남은 잔여물. 루트에서 제거 가능 → **수정 권장**
