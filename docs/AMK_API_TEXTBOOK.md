...
> - **파일명**: `{student|teacher}-{langcode}.webp`. ⚠️ langcode = **api 직렬화 코드(`item.language`) 그대로** — 프론트가 `/covers/{type}-{item.language}.webp` 요청. 대부분 snake_case 이나 **`EsEs`→`es-ES`, `PtPt`→`pt-PT` (serde BCP 47 rename, types.rs:252/256 설계상)** → 파일명도 `student-es-ES.webp`/`teacher-pt-PT.webp` (snake `es_es`/`pt_pt` 아님). `zh_cn`/`zh_tw` 는 serde=snake 라 그대로. (es_es/pt_pt 별도 표지 PDF books 실재.)
