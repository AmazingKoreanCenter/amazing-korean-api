...
- ED: ST (학생용) / TC (교사용)
- PAY: CA (Paddle 카드) / BT (계좌이체) / AP (Apple IAP) / GP (Google IAP)
- NNNN: 일별 순번 (MAX 기반, Advisory Lock으로 동시성 안전)
- DB: `purchase_code VARCHAR(30)` (최대 25자: ZH_CN-TC-20260310-BT-0001)

---

#### 12.5-2.5 : `POST /ebook/purchase/iap` (IAP 구매 확정)

> 모바일 앱에서 RevenueCat SDK로 Apple/Google IAP 결제 완료 후, 영수증을 서버에 전달하여 구매 확정.
> 중복 구매 방지 (동일 user + language + edition, completed 상태).
...
```json
{
  ...
  "edition": "teacher",
  "platform": "apple",
  "product_id": "com.amazingkorean.ebook.vi.teacher",
  "receipt_token": "MIIbzgYJKoZIhvc..."
}
```

- `platform`: `"apple"` 또는 `"google"` — `ebook_payment_method_enum` 확장 값
- `product_id`: App Store / Play Store에 등록된 IAP 상품 ID
- `receipt_token`: RevenueCat SDK가 반환한 영수증 토큰

**응답 (성공 200)**
```json
{
  "purchase_code": "VN-TC-20260407-AP-0001",
  "status": "completed",
  "language": "vi",
  "edition": "teacher",
...
**처리 흐름**:
1. Rate Limit 검사 (기존 ebook purchase와 동일 패턴)
2. 중복 구매 검사 (동일 user + language + edition)
3. RevenueCat REST API로 영수증 검증 (`src/external/revenuecat.rs`)
4. 검증 성공 → `ebook_purchase` 생성 (status=`completed`, payment_method=`apple`/`google`)
5. 구매 확인 이메일 발송 (fire-and-forget)

**실패**:
- **400**: 필수 필드 누락/형식 오류
- **401**: 미인증
- **402**: 영수증 검증 실패 (결제 미완료/무효)
- **409**: 이미 구매 완료 (동일 language + edition)
- **429**: Rate Limit 초과

**환경변수**: `REVENUECAT_API_KEY`, `REVENUECAT_WEBHOOK_SECRET`

**프로덕션 안전장치**: `APP_ENV=production` + `REVENUECAT_API_KEY` 미설정 → 서버 부팅 실패.
`st.revenuecat == None` 이면 영수증 검증 블록이 통째로 건너뛰어지므로 fail-open 으로
결제 우회가 가능해진다. 부팅 단계에서 panic 으로 차단.

**DB 마이그레이션**: `ebook_payment_method_enum`에 `'apple'`, `'google'` 추가

**신규 파일**: `src/external/revenuecat.rs` — RevenueCat REST API 클라이언트
