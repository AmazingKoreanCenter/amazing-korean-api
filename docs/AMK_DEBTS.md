...
| G. 자동 검증 부재 (CI 부채) | ~~5~~ → ~~4~~ → ~~5~~ → ~~4~~ → ~~5~~ → **4** | ~~G3/G4/G5/G6/G7/G11/G13/G14~~ ✅ 해결 또는 🟡 수용 (2026-05-05) + ~~G8~~ ✅ 해결 (2026-05-08) + ~~G15~~ ✅ 해결 (2026-05-10) + ~~G16~~ ✅ 해결 (2026-05-10 옵션 a 정책 정착, migrations/README.md). 잔여 = G1/G2 (보류) + 🟡 G10 src 테스트 부족 (광범위 처리, 166 unit + 7 통합 passed) + G12 cargo-geiger (보류) |
...
| ID | 항목 | 사유 |
|:--:|------|------|
| 🟡 G1 | `cargo test` CI 실행 — **Phase 1+2 보류 진행 (2026-05-10): repo_integration 7 + service_integration 3 = 10 통합 tests #[ignore]** | Phase 1 = `tests/repo_integration.rs` (Postgres only, 7 negative cases). Phase 2 = `tests/service_integration.rs` (verify_email Redis 의존, 3 cases) + `tests/common/mod.rs` 신규 (make_test_state() helper, EMAIL/PAYMENT_PROVIDER=none 채택). 로컬 PostgreSQL + Redis + 5개 환경변수 (DATABASE_URL / REDIS_URL / JWT_SECRET / HMAC_KEY / ENCRYPTION_KEY_V1) 필요. CI 미설정 = 보류 정책 유지. service container 추가 시 활성 가능 |
