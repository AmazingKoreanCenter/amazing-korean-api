...
| G. 자동 검증 부재 (CI 부채) | ~~5~~ → ~~4~~ → ~~5~~ → ~~4~~ → **5** | ~~G3/G4/G5/G6/G7/G11/G13/G14~~ ✅ 해결 또는 🟡 수용 (2026-05-05) + ~~G8~~ ✅ 해결 (2026-05-08) + ~~G15~~ ✅ 해결 (2026-05-10). 잔여 = G1/G2 (보류) + 🟡 G10 src 테스트 부족 (2026-05-10 광범위 처리, 166 unit + 7 통합 passed) + G12 cargo-geiger (보류) + 🟡 **G16 신규** (2026-05-10 후속⁹, sqlx migration 정렬 비호환, fresh DB 셋업 시만 영향) |
...
| ID | 항목 | 사유 |
|:--:|------|------|
| 🟡 G1 | `cargo test` CI 실행 — **Phase 1 보류 진행 (2026-05-10): tests/repo_integration.rs 신규 + 8 통합 tests #[ignore]** | 로컬 PostgreSQL + DATABASE_URL 환경에서만 실행 (`cargo test --test repo_integration -- --ignored`). CI 미설정 = 보류 정책 유지. service container 추가 시 활성 가능 |
...
| 🟡 G16 | migration 정렬 비호환 (2026-05-10 후속⁹ 발견, 통합 테스트 작업 중) | sqlx 의 numeric version 정렬: `20260210_i18n_add_video_content_type.sql` (v=20260210) 이 `20260210000001_i18n_content_translations.sql` (v=20260210000001) 보다 먼저 실행 → ALTER TYPE content_type_enum 시점에 type 미존재 → fresh DB sqlx::test fail. production 점진 적용으로 우회됨 (file rename = `_sqlx_migrations` checksum 깨짐 위험 = 금지). **현재 우회 = `#[tokio::test]` + 수동 PgPool + 기존 amk-pg DB 사용**. 처리 옵션 = (a) 향후 timestamp 길이 통일 / (b) Phase 2/3 시 fresh DB 우회 (기존 DB 또는 transaction rollback 패턴). 영향 = 신규 fresh DB 셋업 시만 (CI service container, dev 환경 신규 셋업) |
