...
| ID | 항목 | 사유 |
|:--:|------|------|
| 🟡 G1 | `cargo test` CI 실행 — **Phase 1+2 진행 (2026-05-10): repo_integration 7 + service_integration 8 = 15 통합 tests #[ignore]** | Phase 1 = `tests/repo_integration.rs` (Postgres only, 7 negative cases). Phase 2 = `tests/service_integration.rs` (3 함수 = verify_email 3 + verify_reset_code 2 + reset_password_with_token 3, 모두 Redis 의존 EmailSender 미사용) + `tests/common/mod.rs` (make_test_state() helper, EMAIL/PAYMENT_PROVIDER=none 채택 = mock 부재). 로컬 PostgreSQL + Redis + 5개 환경변수 필요. CI 미설정 = 보류 정책 유지. Phase 3 (signup/login/oauth/mfa = EmailSender mock 필요) = 별도 트랙 |
