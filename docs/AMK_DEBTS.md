...
| ID | 항목 | 사실 |
|:--:|------|------|
| 🟡 G10 | src/ 테스트 부족 — **광범위 처리 (2026-05-10 후속³): 111 신규, 120 tests** | 2026-05-10 1차 (auth 24) + 후속 (types 5 + payment 5) + 후속² (user 11 + ebook 11 + study 5 + textbook 5 = 32) + 후속³ (video 6 + lesson 10 + user birthday 3 + admin 27 = 46). 누계 = auth 24 + types 5 + payment 5 + user 14 + ebook 11 + study 5 + textbook 5 + video 6 + lesson 10 + admin 27 = **111 신규**. **video 6**: apply_tag_translations (no translations / replace title / fallback count / skip None / subtitle 독립 / multiple tags 합산). **lesson 10**: helper 추출 + test = validate_pagination 4 + compute_total_pages 3 + validate_progress_percent 3. **user 3 (birthday)**: is_valid_birthday 추출 + 1900~today 검증. **admin 27**: textbook status 4 (label / same state 거부 / 모든 쌍 허용 / 역방향 정정) + ebook status 6 (pending→completed/refunded / completed→refunded / Refunded terminal / no rollback / no self-loop) + parse_date_range 7 (valid / trim / invalid format / from>to / 366 days boundary / over 366 / same day) + validate_invite_role 4 (admin/manager OK / 다른 역할 거부 / case-sensitive / error code) + validate_study_idx 6 (≥2 chars / <2 거부 / trim / error code / optional 변형). `cargo test --lib` = **120 passed** / clippy clean / fmt clean. 잔여 = paddle SDK mock 함수 (extract_user_id / event_data_type_name) + service.rs main 비즈니스 (DB/Redis 의존 = G1/G2 통합 테스트 트랙) + 추가 admin 도메인 함수. G1/G2 (cargo test CI) 보류 별개 |
