...
| ID | 항목 | 사실 |
|:--:|------|------|
| 🟡 G10 | src/ 테스트 부족 — **광범위 처리 (2026-05-10 후속⁴): 128 신규, 137 tests** | 2026-05-10 누계 = auth 24 + types 5 + payment 5 + user 14 + ebook 11 + study 5 + textbook 5 + video 6 + lesson 10 + admin 44 = **128 신규** + 기존 9 = **137 tests**. **후속⁴ (admin 추가 17)** = lesson/repo normalize_lesson_action 4 (create/update/delete variants + unknown→update fallback) + video/repo normalize_video_action 2 (create+bulk_create / fallback) + study/repo normalize_study_action 4 (create / state transitions = banned/reorder/publish/unpublish / update default / unknown) + admin/lesson/handler extract_client_ip 5 (x-forwarded-for first / x-real-ip fallback / x-forwarded 우선 / missing → None / invalid format → None) + extract_user_agent 2. 4 도메인 동일 extract_* 함수 = lesson 대표만 test, 다른 3 도메인 = 공통 추출 후속 트랙 (별도). 잔여 = paddle SDK mock + service.rs main 비즈니스 (G1/G2 통합 트랙) + service.rs 추가 helper 추출 (auth signup 입력 정규화 / OAuth state / TOTP / MFA backup mask). G1/G2 (cargo test CI) 보류 별개 |
