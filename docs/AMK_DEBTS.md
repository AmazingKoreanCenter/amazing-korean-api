...
| ID | 항목 | 사실 |
|:--:|------|------|
| 🟡 G10 | src/ 테스트 부족 — **부분 처리 (2026-05-10): auth 24 신규** | 2026-05-10 = `src/api/auth/{password,jwt,service}.rs` 24 신규 (password 6 = argon2id 형식 / unique salt / round-trip / unicode / wrong password / malformed hash + jwt 7 = create-decode roundtrip / wrong secret / malformed / expired / unique jti / ttl→expires_in / RFC3339 + service 11 = refresh token generate-decode / unique per call / hash 일치 / parse 추출 / invalid base64 / missing colon / non-UUID session/random / empty session / invalid UTF-8). `cargo test --lib` = **33 passed** (api::auth 24 + 기존 docs/external/* 9). 잔여 = 다른 도메인 (user / payment / ebook / video / study / lesson / textbook), service.rs 의 비-pure 함수 (Redis/DB 의존). 메모리 "4건" stale 정정 (실제 기존 9건 + 신규 24 = 33). G1/G2 (cargo test CI) 보류 별개 |
