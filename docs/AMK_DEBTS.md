...
| ID | 항목 | 사실 |
|:--:|------|------|
| 🟡 G10 | src/ 테스트 부족 — **광범위 처리 (2026-05-10 후속²): auth 24 + types 5 + payment 5 + user 11 + ebook 11 + study 5 + textbook 5 = 66 신규, 75 tests** | 2026-05-10 1차 = auth 24. 2026-05-10 후속 = types 5 + payment 5. 2026-05-10 후속² = user 11 (validate_password_policy 6 = 8+chars+letter+digit / hmac_verification_code 5 = 결정성·다른입력→다른출력) + ebook 11 (to_korean_title 5 = 영→한 TOC / language_name_ko 1 / edition_label_ko 1 / constant_time_eq 4 = 타이밍 공격 방지) + study 5 (content_type_for_task_kind 1 / parse_study_program 3 / invalid messages 2) + textbook 5 (language_display_name 1 / catalog_languages 4 = count·isbn·available·name 회귀). `cargo test --lib` = **75 passed**. video = pure helper 없음 skip (apply_tag_translations = struct mock 비용 ↑). lesson = 모두 async DB skip. 잔여 = payment 비-pure 함수 (paddle SDK mock) + admin 도메인 + 다른 비-pure 함수. G1/G2 (cargo test CI) 보류 별개 |
