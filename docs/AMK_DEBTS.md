...
| G2-1 | e2e vite dev cold start 안정화 (login_flow.spec dormant 해제) | 🟡 **신규 (2026-05-11, PR #272)**. login_flow.spec 추가 후 CI 2회 fail (60s → 90s timeout 확장 후에도 fail). 원인 = login page React.lazy chunk 의 vite dev cold-compile + JS 로딩이 alphabetical 첫 spec 에서 안정 보장 어려움 (writing_practice.spec 는 warm path = 3.5s pass). 본 spec 은 `test.describe.skip` 으로 dormant 마킹 (코드 보존). 해제 조건 = (a) vite preview + build + playwright `webServer` option, 또는 (b) dev warmup beforeAll page.goto, 또는 (c) frontend chunk 사전 컴파일 step. 추정 0.3-0.5일 |
