...
| ID | 항목 | 사실 |
|:--:|------|------|
| 🟡 G10 | src/ 테스트 부족 — **부분 처리 (2026-05-10 후속): auth 24 + types 5 + payment 5 = 34 신규, 43 tests** | 2026-05-10 1차 = `src/api/auth/{password,jwt,service}.rs` 24 신규. 2026-05-10 후속 = `src/types.rs` BillingInterval 5 (months / price_cents / Display / 단조성 / 월 단가) + `src/api/payment/service.rs` paddle_status_to_internal 5 (5 variant 매핑). `cargo test --lib` = **43 passed** (auth 24 + types 5 + payment 5 + 기존 docs/external 9). 잔여 = payment service.rs 의 비-pure 함수 (extract_user_id / event_data_type_name = paddle SDK mock 필요) + 다른 도메인 (user / ebook / video / study / lesson / textbook). 메모리 "4건" stale 정정 = 실제 기존 9건. G1/G2 (cargo test CI) 보류 별개 |
