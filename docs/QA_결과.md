...
- [ ] **2.1 ebook subtitle** 수정 예정? (예 / 아니오 / 언제까지)
- [ ] **2.2 textbook subtitle** 수정 예정? (예 / 아니오 / 언제까지)
- [ ] **2.3 pt footer 오버랩** 수정 예정? (예 / 아니오 / 우선순위 낮음)
- [ ] **2.4 캐러셀 dot aria-label** 수정 예정? (예 / 아니오)
- [ ] **3.1 JWT TTL** — (A) QA env 연장 / (B) refresh 플로우 공유 / 현재 TTL 값 알려주기
- [ ] **3.2 OpenAPI drift** — (A) overrides 공동 관리 / (B) QA 가 drift tolerance 로 전환
