...
| 3-2a | `POST /auth/logout/all` | (전역처리) | 전체 로그아웃 | ***사용자의 모든 세션/리프레시 키 일괄 제거, LOGIN_LOG 저장***<br>성공: Auth pass / Request logout_all pending→success → **200**<br>실패(미인증): Auth stop → **401** | [✅] |
...
- 성공 → **200**
  - When: `/find-id`에서 식별 정보(이름 + 생년월일)를 입력하고 제출한다
