...
| Q1c | **높음** | **응답 스키마 최종 정렬** — 덮어쓰기 유지 vs `_translated` 접미사 방식 사용자 최종 결정 + `translation_lang`/`translation_coverage` 메타 필드 추가 여부 + **Video 테이블 title/subtitle 물리 컬럼 추가 여부** + **video_tag 번역 주입 설계** (response 에 `video_tag_id` 노출 필요). 프론트 소비 0건 확인돼 회귀 리스크 없음. | 0.5~1일 | Q1a/Q1b 완료 후. 최종 구현과 §9-841 스펙 일치 보장 |
