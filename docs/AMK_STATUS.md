...
| 67 | E-book session_id 필수화 — Phase 1 관측 로깅 배포 | 보안 | `verify_session(session_id: Option<&str>)` 의 `Option` 제거 + `None → Forbidden` 전환 전 프로덕션 트래픽 표본으로 미전송 케이스 0건 확인 단계. **Phase 1 배포**: `src/api/ebook/service.rs:504` 진입부에 `session_id.is_none()` 분기로 `tracing::warn!(user_id, "EBOOK_SESSION_AUDIT: verify_session called without x-ebook-session header")` 추가. 기존 로직(미제공 시 Redis 키 존재만 확인)은 그대로 유지. doc `TODO` 에 전환 목표일 `2026-04-24` 명시. **모바일/데스크탑 사전 grep**: 모바일(`lib/api/ebook_api.dart:50,68`) `required String sessionId` 항상 전송 → 안전. 데스크탑(`src/category/ebook/ebook_api.ts:76,92,114,132`) 은 웹과 동일한 optional 패턴 — meta 로드 후엔 항상 전송되나 D+8 Phase 2 전환 시 데스크탑 `ebook_api.ts` + `use_page_image.ts` + `ebook_viewer_page.tsx` 도 필수화 처리 필요. **관측 계획**: 5~7일(D+0 ~ D+7=2026-04-23). 완료 조건 `docker logs amk-api --since 168h 2>&1 \| grep EBOOK_SESSION_AUDIT \| wc -l → 0건`. **Phase 2~5 일괄 전환**: D+8=2026-04-24 (백+웹+데스크탑 동일 PR 동시 배포). **관측 모드 선택 근거**: INC-001 (2026-04-15 프로덕션 2h33m 다운) 경험으로 "fail-closed 게이트 추가는 코드 분석만 신뢰 말고 프로덕션 로그로 선확인" 방침 확립 | 2026-04-16 (Phase 1만, Phase 2~5 는 D+8 예정) | Phase 2~5 전환 완료 시 재갱신 |
