...
| Q13 | **중간** | **`supported_language_enum` 확장 + UI locale 보강 (es_es / pt_pt + 13 누락)** — 2026-04-21 "pt_pt → pt 병합 정책" 번복 (사용자 결정 2026-04-27). books `sentences.json` 35 언어 전부 DB enum 으로 수용 + frontend UI locale 22 → 35 확장. ✅ **Phase 1 완료 (2026-04-28)** — `migrations/20260428_add_es_pt_variants.sql` (ALTER TYPE +es_es/pt_pt), `src/types.rs` `SupportedLanguage` enum +EsEs/PtPt (sqlx 'es_es'/'pt_pt', serde 'es-ES'/'pt-PT'), 기존 마이그 20260421 주석에 정책 번복 노트, 문서 4개 (AMK_SCHEMA_PATCHED L587 22→37, AMK_API_MASTER L1334 35→37 + es-ES/pt-PT 추가, AMK_API_LEARNING L605 21→36 + 그룹 재구성, L919 6,300→10,800). cargo check + clippy --lib --bins -D warnings 클린. → **Phase 2** (frontend UI locale 15 신규, 자동 번역 22,500 키, 8-16시간, 별도 세션 3 PR 분할 권장) → **Phase 3** (메타 표시, 1-2시간) → **Phase 4** (books `gen_seed_sql.py` 의 es_es/pt_pt skip 로직 제거, books 측 handoff). 신규 마이그레이션 prefix `20260428_` (INC-003 충돌 회피). **TextbookLanguage enum 21 확장 여부는 별도 결정**. | Phase 1 완료, 잔여 9-18시간 (3-4 세션) | **plan SSoT**: `~/.claude/plans/supported-language-es-pt-variants-expansion.md` |
