...
| 20 | JsonRejection → AppError 통합 | 코드 품질 | `Json<T>` extractor 실패 시 Axum 기본 `text/plain` 응답을 커스텀 추출기로 가로채 AppError(`Unprocessable`/`BadRequest`) 로 매핑 | 에러 응답 envelope + `trace_id` 전 경로 통일 | #69 trace_id 검증에서 발견 (2026-04-16), 우선순위 낮음 |
