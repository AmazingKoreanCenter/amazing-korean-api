...
| 65 | SEO hardening — Cloudflare Managed robots.txt 우회 | SEO/보안 | **배경**: #64 배포 후 외부 검증에서 Cloudflare 가 zone 레벨로 `# BEGIN Cloudflare Managed content` 블록을 robots.txt 본문 앞에 자동 주입 발견. 주입 블록의 `User-agent: *` + `Allow: /` 가 우리 Disallow 와 tie-breaking 에서 Allow 승리로 무력화. **Fix A**: `src/api/mod.rs::robots_txt` 에 명시적 크롤러 10종(Googlebot, Googlebot-Image/News/Video, AdsBot-Google, Bingbot, DuckDuckBot, Yeti, NaverBot, Daum) 각각 `Disallow: /` + `*` 폴백. Google 의 "더 구체적인 user-agent 그룹 우선" 규칙으로 Cloudflare `*` 우회. **Fix B-2**: `src/main.rs::security_headers` 미들웨어에 `x-robots-tag: noindex, nofollow` 추가. 전역 Axum `.layer()` 이므로 **api 전체 응답에 자동 주입**. HTTP 헤더는 Cloudflare 가 수정 불가 → robots.txt 와 독립 작동하는 2중 방어. **Fix C (사용자 후속)**: Cloudflare Zone 설정에서 Managed robots.txt 주입 토글 확인/조정 예정. **검증**: `cargo check` + `cargo clippy --lib` 0 warnings | 2026-04-15 | Cloudflare zone 설정 후속 (사용자) + 향후 공개 색인 엔드포인트 추가 시 x-robots-tag override |
