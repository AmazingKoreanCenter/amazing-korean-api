...
| 72 | 교재 500문장 시딩 선행 마이그레이션 (단일 PR / M01~M04 누적 진행) | 콘텐츠/스키마 | **목적**: 500문장 × 36언어 번역 시딩(M05~M08) 전 스키마 정리. 총 8개 마이그레이션 중 저위험 스키마 확장 2건만 우선. **M01 `20260418_study_task_idx.sql`**: `study_task` 에 `study_task_idx varchar(100) NOT NULL UNIQUE` 컬럼 추가. 해설집 문장 참조 안정 키 + 재시딩 멱등성 + `content_translations.content_id` 논리 연결 안정성 3가지 요구를 단일 컬럼으로 해결. 기존 레거시 행은 `'legacy-' \|\| study_task_id` 로 백필 (후속 M02 에서 전량 삭제 예정). **M04 `20260421_expand_supported_languages.sql`**: `supported_language_enum` 에 13개 언어 추가 (`tl`/`tr`/`bn`/`ar`/`ur`/`fa`/`lo`/`ky`/`it`/`sw`/`uk`/`am`/`pl`). sentences.json 36개 번역 중 기존 enum(22) 미지원 13개 커버. `pt_pt` → `pt` 병합. **Rust 동기화**: `src/types.rs` `SupportedLanguage` enum 에 13 variant 추가 (주석 "21개 → 35개"). **문서**: `AMK_API_MASTER §4.8` + `AMK_SCHEMA_PATCHED §2.4.2` + `AMK_CHANGELOG` 동기화. **검증**: `cargo check` ✅, `cargo clippy --all-targets -- -D warnings` 0 warnings ✅, `cargo sqlx prepare` (변경 없음 — 신규 enum 값은 미사용) ✅, 로컬 `docker exec amk-pg` 에 M01+M04 적용 성공 (`\d study_task` 컬럼+UNIQUE, `enum_range` 35개 확인) ✅ | 2026-04-18 | PR #2 = M02 (test-* 리셋), PR #3 = M03 (basic_900 → basic_500 개명) 후 M05~M08 시딩 본체 |
