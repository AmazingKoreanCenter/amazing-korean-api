...
- **Q16 — Frontend ESLint baseline cleanup** (2026-05-04 발견, 사용자 트리거 대기): PR-check workflow (`pr-check.yml`) 도입 시 main baseline 의 ESLint 위반 27 errors 검출. 대부분 = `react-refresh/only-export-components` (shadcn/ui 컴포넌트 + variants 같은 파일 export 패턴이 새 plugin 룰과 충돌). 일부 = `react-hooks/set-state-in-effect`, `react-hooks/incompatible-library`, `prefer-const`. 작업 = (1) shadcn 컴포넌트 파일 분할 (예: `button.tsx` → `button.tsx` + `button-variants.ts`), (2) react-hooks 룰 위반 useEffect 리팩터, (3) prefer-const 1건 fix. 시간 추정 1-2일 (전수 수정 + 회귀 테스트). 임시 조치 (PR #205) = `pr-check.yml` 의 `npm run lint` 단계 `continue-on-error: true` (lint 결과 표시되나 fail 처리 X), `lint:ui` (하드코딩 색상) 는 pass 라 정상 활성. 본 작업 완료 후 `continue-on-error` 제거.
