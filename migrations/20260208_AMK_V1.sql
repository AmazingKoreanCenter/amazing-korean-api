-- AMK V1 통합 마이그레이션 (2026-02-08)
-- 모든 마이그레이션을 하나로 통합 (최종 스키마)
...
CREATE TABLE course (
  ...
  course_idx VARCHAR(100) UNIQUE NOT NULL,
  course_title VARCHAR(150) NOT NULL,
  course_subtitle VARCHAR(250),
...
CREATE TABLE course_lesson (
  ...
  lesson_id INT NOT NULL,
  course_lesson_seq INT NOT NULL DEFAULT 1,
...
CREATE TABLE user_course (
  ...
  user_id BIGINT NOT NULL,
  course_id INT NOT NULL,
  user_course_active BOOLEAN NOT NULL DEFAULT true,
  user_course_start_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  user_course_expire_at TIMESTAMPTZ,
  user_course_progress_percent INT NOT NULL DEFAULT 0,
  user_course_last_lesson_id INT,
  user_course_last_progress_at TIMESTAMPTZ,
  user_course_granted_by_user_id BIGINT,
  user_course_pay_id BIGINT,
...
ALTER TABLE user_oauth ADD CONSTRAINT fk_user_oauth_user FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE;
...
ALTER TABLE study_task_log ADD CONSTRAINT fk_study_task_log_login FOREIGN KEY (login_id) REFERENCES login (login_id);
...
-- 4. Indexes
CREATE UNIQUE INDEX unique_users_email_idx ON users(user_email_idx);
CREATE INDEX idx_users_name_idx ON users(user_name_idx);
CREATE INDEX idx_user_oauth_user_id ON user_oauth(user_id);
CREATE INDEX idx_oauth_subject_idx ON user_oauth(oauth_subject_idx);
CREATE UNIQUE INDEX unique_oauth_provider_subject_idx ON user_oauth(oauth_provider, oauth_subject_idx);
...
CREATE INDEX index_study_state_access ON study (study_state, study_access);
...
CREATE INDEX index_lesson_state_access ON lesson (lesson_state, lesson_access);
